...
if(file.exists(file_path)){
    ...
    filler_data <- data %>%
      filter(V16 == "filler") %>%
      select(Part_ID = V13, Correct_Resp = V19, Participant_Response = V20) %>%
      mutate(Is_Correct = ifelse(Participant_Response == Correct_Resp, 1, 0)) %>%
      group_by(Part_ID) %>%
      summarise(accuracy = mean(Is_Correct, na.rm = TRUE))
    
    bad_participants <- filler_data %>% filter(accuracy < 0.80) %>% pull(Part_ID)
